...
function SearchBar() {
  ...
  const fetchChallenges = async (query = '', selectedCategory = '', lastPlanetId = null, isLoadMore = false) => {
    try {
      ...
      if (isLoadMore) {
        setDisplayedChallenges((prevChallenges) => [...prevChallenges, ...formattedChallenges]);
      } else {
      ...
      if (formattedChallenges.length > 0) {
        setLastPlanetId(formattedChallenges[formattedChallenges.length - 1].planetId);
      }
                ...
                {displayedChallenges.map((challenge) => (
                  <li className='challengeList' key={challenge.planetId}>
                    ...
                    <ul className='challengeExp'>
                      ...
                      <li>{challenge.startDate} ~ {challenge.endDate}</li>
                      <li>{challenge.currentParticipants} / {challenge.maxParticipants}</li>
                    </ul>
                  </li>
                ))}
              </ul>
                    ...
                    <ul className='challengeExp'>
                      ...
                      <li>
                        <div class = 'date_participant'>
                          {challenge.startDate} ~ {challenge.endDate} | {challenge.currentParticipants} / {challenge.maxParticipants}
                        </div>
                      </li>
                      <li>{challenge.currentParticipants} / {challenge.maxParticipants}</li>
                    </ul>
