...
function SearchBar() {
  ...
  const fetchChallenges = async () => {
    try {
      const response = await axios.get('http://70.12.114.73:8080/api/v1/planets', {
        params: {
          size: 5,
          ...
          <ul>
            {filteredChallenges.map((challenge) => (
              <li className='challengeList' key={challenge.id}>
                <img className='challengeImg' src={challenge.planetImage} alt={challenge.planetName} width={50} />
