    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      if (refreshToken) {
        try {
          ...
          sessionStorage.setItem('access_token', newAccessToken); // 새로운 액세스 토큰을 세션 스토리지에 저장
          ...
          // 토큰 갱신 실패 시 로그아웃 처리
          sessionStorage.removeItem('access_token'); // 세션 스토리지에서 액세스 토큰 제거
          sessionStorage.removeItem('refresh_token'); // 세션 스토리지에서 리프레시 토큰 제거
          // 리디렉션 또는 로그아웃 로직 추가
